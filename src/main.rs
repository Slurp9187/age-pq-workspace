...
fn keygen(output: Option<String>, native: bool) -> io::Result<()> {
    ...
    let identity_hrp = Hrp::parse(identity_hrp_str)
        .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
    let identity = bech32_encode::<Bech32>(identity_hrp, seed.as_ref())
        .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?
        .to_uppercase();

    let output_text = format!("# created: {created}\n# public key: {recipient}\n{identity}");
...
fn convert_native_identities() -> io::Result<()> {
    ...
    for line in input.lines() {
        ...
        let plugin_hrp = Hrp::parse(IDENTITY_BECH32_HRP)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
        let plugin_identity = bech32_encode::<Bech32>(plugin_hrp, seed.as_ref())
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?
            .to_uppercase();
        println!("{}", plugin_identity);
    }
