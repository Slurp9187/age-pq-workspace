...
> **Frozen.** This file is the historical record for `age-pq-hpke` on the MSRV-1.70
> line, through `0.1.0-rc.1`. Any future `0.1.x` patch is recorded in the
> [root `CHANGELOG.md`](../CHANGELOG.md) under a `### age-pq-hpke` heading.
>
...
- `docs/plans/age-pq-hpke-secure-gate-hardening.md` (PRs 1–5).
...
- Renamed crate and GitHub repository from `pq-xwing-hpke` to `age-pq-hpke` (import as `age_pq_hpke`); `repository` URL and docs aligned with the new name.
